...
```mermaid
flowchart TD
    A[1. КЛИЕНТ<br/>Заявка через веб-чат<br/>или Telegram] --> B[AI-АГЕНТ МЕНЕДЖЕРА<br/>по работе с клиентами<br/>• Извлечение сущностей<br/>• Автоматическое обезличивание<br/>• Присвоение кода AB-CD/F<br/>• RLS-изоляция ПДн]

    B --> C[2. РУКОВОДИТЕЛЬ<br/>Главацкая И.В.<br/>Визирование в системе<br/>(ЭЦП / логин)]

    C --> D[3. НАЧ. ЛАБА<br/>Малицкая Ю.А.<br/>Маршрутизация через систему<br/>AI подсказывает методику из RAG]

    D --> E[4. AI-АГЕНТ МЕНЕДЖЕРА<br/>ДОКУМЕНТООБОРОТА<br/>• Автоматическое заполнение<br/>  первого листа Ф18<br/>• Draft_AI промежуточного протокола]

    E --> F{6. AI-АГЕНТ БУХГАЛТЕРА<br/>Автоматическая сверка с 1С<br/>по веб-хуку от банка}

    F -->|Вариант А: предоплата| G[5. AI-АГЕНТ ИНЖЕНЕРА-ХИМИКА<br/>• Автоподстановка методики из RAG<br/>• Детерминированный движок расчётов<br/>• Автоматическая сверка с СанПиН<br/>• Проверка сроков поверки СИ]
    F -->|Вариант Б: постоплата| G

    G --> H[5а. AI-АГЕНТ ЛАБОРАНТА<br/>• Учёт реактивов<br/>• Сканирование штрих-кодов<br/>• Контроль остатков]

    H --> I[7. НАЧ. ЛАБА<br/>Визирование в системе<br/>AI подсвечивает отклонения]

    I --> J[8. AI-АГЕНТ МЕНЕДЖЕРА<br/>ДОКУМЕНТООБОРОТА<br/>• Агрегация промежуточных протоколов<br/>• Draft_AI итогового протокола<br/>за 10 секунд (вместо 40 мин)]

    J --> K[9. КЛИЕНТ<br/>Получение итогового протокола<br/>через систему / email]
